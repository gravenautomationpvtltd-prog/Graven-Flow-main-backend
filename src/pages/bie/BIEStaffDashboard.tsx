...
import { AlertTriangle, CalendarClock, CheckCircle2, ClipboardList, ListTodo, RotateCcw } from 'lucide-react';
...
export default function BIEStaffDashboard() {
  ...
  const rework = rows.filter((row) => row.review_status === 'rework');

  const cards = [
    { label: 'My open work', value: open.length, icon: ListTodo },
    { label: 'Due in 7 days', value: dueSoon.length, icon: CalendarClock },
    { label: 'Overdue', value: overdue.length, icon: AlertTriangle },
    { label: 'Waiting for review', value: inReview.length, icon: CheckCircle2 },
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">The work assigned to you, your deadlines and progress.</p>
        </div>
        <Button asChild>
          <Link to="/bie/my-work"><ClipboardList className="mr-2 h-4 w-4" />Open my work</Link>
        </Button>
      </div>
        ...
        <Card className="border-destructive/50">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-destructive">
              <RotateCcw className="h-5 w-5" />Sent back to you ({rework.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {rework.map((row) => (
              <div key={`${row.type}-${row.id}`} className="rounded-md border p-3">
                <p className="font-medium">{row.title}</p>
                <p className="text-sm text-muted-foreground">{row.rework_note || 'Your manager asked for changes.'}</p>
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
              ...
              <div key={`${row.type}-${row.id}`} className="flex items-center justify-between rounded-md border p-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {typeLabels[row.type]} · given by {nameOf(row.assigned_by)}
                  </p>
                </div>
        ...
        <Card>
          <CardHeader><CardTitle>My recent updates</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {[...rows]
              ...
              .map((row) => (
                <div key={`${row.type}-${row.id}`} className="flex items-center justify-between rounded-md border p-3">
                  <div>
                    <p className="font-medium">{row.title}</p>
                    <p className="text-sm text-muted-foreground">{typeLabels[row.type]} · {row.subtitle}</p>
                  </div>
                  <Badge variant={row.completed_at ? 'default' : 'secondary'}>{row.status.replace(/_/g, ' ')}</Badge>
                </div>
              ))}
            {!rows.length && <p className="py-8 text-center text-muted-foreground">No work assigned to you yet</p>}
          </CardContent>
        </Card>
      </div>
    </div>

-- Fast SECURITY DEFINER RPCs for the LQT inbox.
-- Replaces multi-step client fetches that were timing out under RLS fan-out.
-- All visibility is enforced inside the function via tenant + role checks.
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE user_id = v_user AND role = 'cro'::app_role
  ) INTO v_is_lqt;
  ...
  -- Routed tabs: nurture / qualified / discarded
  SELECT COALESCE(jsonb_agg(row_data ORDER BY (row_data->'qualification'->>'qualified_at') DESC), '[]'::jsonb)
  INTO v_rows
  FROM (
    SELECT jsonb_build_object(
      'id', l.id,
...
GRANT EXECUTE ON FUNCTION public.get_lqt_inbox(text, timestamptz, timestamptz, int) TO authenticated;
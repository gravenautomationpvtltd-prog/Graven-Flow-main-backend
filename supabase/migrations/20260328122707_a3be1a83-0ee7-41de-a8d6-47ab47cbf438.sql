...
USING (
  ...
  AND (
    ...
    OR EXISTS (SELECT 1 FROM cro_customer_assignments WHERE cro_customer_assignments.customer_id = customers.id AND cro_customer_assignments.cro_user_id = auth.uid())

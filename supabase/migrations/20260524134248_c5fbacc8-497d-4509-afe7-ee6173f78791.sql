...
-- Fix LQT round-robin: remove hard-coded Sweta fallback, prioritise "fewest leads today" for fairness.

CREATE OR REPLACE FUNCTION public.pick_next_lqt_user(p_tenant_id uuid, p_vertical_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  v_pick uuid;
BEGIN
  -- 1) Presence-filtered, vertical-scoped pool, ordered by TODAY's load first
  ...
  ORDER BY assigned_today ASC, open_pending ASC, last_assigned_at ASC NULLS FIRST, user_id ASC
  LIMIT 1;

  IF v_pick IS NOT NULL THEN RETURN v_pick; END IF;

  -- 2) Vertical-scoped fallback ignoring presence (early morning, before check-ins)
  SELECT user_id INTO v_pick
  FROM (
    SELECT p.id AS user_id,
           COUNT(l.id) FILTER (
  ...
  -- 3) Cross-vertical safety net (any active CRO in the tenant)
...
AS $function$
BEGIN
  RETURN public.pick_next_lqt_user(p_tenant_id, NULL::uuid);
END;
$function$;
...
BEGIN
  ...
  -- Primary: presence-aware, today-balanced picker
  v_lqt_pick := public.pick_next_lqt_user(NEW.tenant_id, NEW.vertical_id);

  -- If no one matched (e.g. early morning, nobody checked in yet),
  -- fall back to fewest-leads-today among active CROs in this tenant/vertical
  -- IGNORING attendance, so webhooks at 3am still distribute fairly.
  ...
  -- Widen to tenant (any vertical) if still nothing
  ...
  -- Assign if we found one; otherwise leave NULL (surfaces as unassigned in LQT)

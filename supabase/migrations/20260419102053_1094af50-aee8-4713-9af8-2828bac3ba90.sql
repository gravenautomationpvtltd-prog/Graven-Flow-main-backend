DROP FUNCTION IF EXISTS public.get_spt_inbox();

CREATE OR REPLACE FUNCTION public.get_spt_inbox()
 RETURNS TABLE(id uuid, title text, customer_query text, created_at timestamp with time zone, has_enquiry boolean, enquiry_status text, source text, assigned_to uuid, owner_name text, customer_id uuid, customer_company text, customer_contact text, customer_phone text, customer_email text, customer_segment text, qualification_type text, routed_to text, qualified_at timestamp with time zone, qualifier_name text, item_count integer, total_qty numeric, pending_pricing integer, verified_auto integer, updated_pricing integer, latest_quotation_id uuid, latest_quotation_status text, latest_quotation_sent_at timestamp with time zone, latest_quotation_created_at timestamp with time zone, latest_quotation_grand_total numeric, handoff_at timestamp with time zone, stage text, pricing_mode text, lead_status text, won_at timestamp with time zone, lost_at timestamp with time zone, estimated_value numeric)
 LANGUAGE plpgsql
...
BEGIN
  ...
  WITH base AS (
    SELECT l.*
    FROM public.leads l
    INNER JOIN public.lead_qualification lq_gate
      ON lq_gate.lead_id = l.id
     AND lq_gate.routed_to::text IN ('spt','tst')
     AND lq_gate.qualified_at IS NOT NULL

...
BEGIN
  -- Reassign leads (removed invalid 'cancelled' from lead_status enum filter)
  ...
  -- Reassign CRO customer assignments (expanded status filter)
  UPDATE cro_customer_assignments SET cro_user_id = p_to_user_id
  WHERE cro_user_id = p_from_user_id
    AND status IN ('active', 'pending', 'contacted', 'no_response');

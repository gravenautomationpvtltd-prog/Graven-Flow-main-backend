-- Extend redistribute_user_book to accept any recipient role (sales, cro, etc.)
-- Keep existing 5-arg signature working by wrapping it.
...
CREATE OR REPLACE FUNCTION public.redistribute_user_book(
  ...
  p_recipient_role text DEFAULT 'sales'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Validate recipient role string
  BEGIN
  ...
  -- Recipients must share tenant, be active, and hold the requested role
  ...
  IF 'leads' = ANY(p_entities) THEN
    ...
    log AS (
      ...
      SELECT v_caller, 'bulk_redistribution', 'Lead reassigned via bulk redistribution',
        jsonb_build_object('batch_id',v_batch_id,'from_user_id',p_from_user_id,'to_user_id',u.new_owner,'entity','lead','source','admin_redistribute_tool','recipient_role',p_recipient_role),
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'leads_moved', v_leads_moved, 'customers_moved', v_customers_moved,
  ...
  RETURN jsonb_build_object(
    'batch_id', v_batch_id, 'leads', v_leads_moved, 'customers', v_customers_moved,
...
GRANT EXECUTE ON FUNCTION public.redistribute_user_book(uuid, uuid[], text[], boolean, boolean, text) TO authenticated;
-- Tighten SPT inbox: require completed LQT/TST qualification handoff
...
BEGIN
  ...
  WITH base AS (
    SELECT l.*
    FROM public.leads l
    -- STRICT GATE: lead must have a completed LQT/TST qualification handed off to SPT/TST
...
-- Defense in depth: force every newly inserted lead to start un-qualified.
-- LQT (or TST) must explicitly flip has_enquiry=true during qualification handoff.
...
AS $$
BEGIN
  -- Force new leads to land un-enquired so SPT inbox cannot see them
  -- until LQT/TST has produced a lead_qualification row.
...
CREATE TRIGGER trg_enforce_lqt_landing_zone
BEFORE INSERT ON public.leads
FOR EACH ROW
EXECUTE FUNCTION public.enforce_lqt_landing_zone();
-- Create a function for intelligent duplicate customer detection with weighted scoring
CREATE OR REPLACE FUNCTION public.find_potential_duplicate_customer(
  p_phone text,
...
BEGIN
  ...
  SELECT 
    ...
    (
      -- GST match: 100 points (definitive)
      ...
      THEN 40 ELSE 0 END
      +
      -- Company name + State match: 30 points
      ...
      THEN 30 ELSE 0 END
      +
      -- Company name + City match: 25 points
      CASE WHEN p_company_name IS NOT NULL 
           AND cm.company_name IS NOT NULL
      ...
      THEN 25 ELSE 0 END
      +
      -- Company name only match: 10 points (just a warning)
      CASE WHEN p_company_name IS NOT NULL 
           AND cm.company_name IS NOT NULL
           AND LOWER(TRIM(p_company_name)) = LOWER(TRIM(cm.company_name))

-- 1. Drop the duplicate trigger + its function
DROP TRIGGER IF EXISTS trg_enquiry_auto_price_request ON public.enquiry_items;
DROP FUNCTION IF EXISTS public.trg_fn_enquiry_auto_price_request();

-- 2. Deduplicate existing price_requests per enquiry_item_id
-- Keep the OLDEST row; merge resolved data forward if any duplicate is resolved.
WITH ranked AS (
  SELECT id, enquiry_item_id,
         ROW_NUMBER() OVER (PARTITION BY enquiry_item_id ORDER BY created_at ASC, id ASC) AS rn
  FROM public.price_requests
  WHERE enquiry_item_id IS NOT NULL
),
keepers AS (
  SELECT enquiry_item_id, id AS keep_id FROM ranked WHERE rn = 1
),
resolved_dupes AS (
  SELECT DISTINCT ON (pr.enquiry_item_id)
         pr.enquiry_item_id,
         pr.status, pr.resolved_at, pr.resolved_by, pr.resolved_price, pr.supplier_id, pr.notes
  FROM public.price_requests pr
  JOIN ranked r ON r.id = pr.id
  WHERE r.rn > 1 AND pr.status = 'resolved'
  ORDER BY pr.enquiry_item_id, pr.resolved_at DESC NULLS LAST
)
UPDATE public.price_requests p
SET status = COALESCE(rd.status, p.status),
    resolved_at = COALESCE(p.resolved_at, rd.resolved_at),
    resolved_by = COALESCE(p.resolved_by, rd.resolved_by),
    resolved_price = COALESCE(p.resolved_price, rd.resolved_price),
    supplier_id = COALESCE(p.supplier_id, rd.supplier_id),
    notes = COALESCE(p.notes, rd.notes)
FROM keepers k
JOIN resolved_dupes rd ON rd.enquiry_item_id = k.enquiry_item_id
WHERE p.id = k.keep_id AND p.status <> 'resolved';

-- Now delete duplicates (keep oldest)
DELETE FROM public.price_requests pr
USING (
  SELECT id, ROW_NUMBER() OVER (PARTITION BY enquiry_item_id ORDER BY created_at ASC, id ASC) AS rn
  FROM public.price_requests
  WHERE enquiry_item_id IS NOT NULL
) d
WHERE pr.id = d.id AND d.rn > 1;

-- 3. Enforce uniqueness so duplicates can never recur
CREATE UNIQUE INDEX IF NOT EXISTS price_requests_enquiry_item_unique
  ON public.price_requests(enquiry_item_id)
  WHERE enquiry_item_id IS NOT NULL;

-- 4. Harden the single remaining auto-create trigger function
...
DECLARE
  ...
  v_assigned uuid;
BEGIN
  -- Only auto-create for free-text items (no matched product)
...
-- 5. Backfill missing tenant_id on existing price_requests (root-cause fix for procurement visibility)
...
-- 6. Update get_spt_inbox so a pending procurement request forces 'awaiting' state.
-- This guarantees Sales Inbox and Procurement Queue stay in sync from the same persisted state.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      -- Open procurement request → always awaiting (single source of truth with Procurement Queue)

...
DECLARE
  ...
  v_escalations int := 0;
  v_reminders int := 0;
  v_sales_roles boolean := false;
BEGIN
  IF NOT (public.is_manager_or_above(auth.uid()) AND public.is_same_tenant(p_user_id)) THEN
    RAISE EXCEPTION 'Permission denied';
  END IF;

  v_sales_roles := ('sales'::app_role = ANY(p_roles_removed))
                OR ('cro'::app_role   = ANY(p_roles_removed));
  ...
  IF v_sales_roles OR ('manager'::app_role = ANY(p_roles_removed)) THEN
    SELECT count(*) INTO v_escalations FROM escalation_logs
      WHERE user_id = p_user_id AND resolved_at IS NULL;

    SELECT count(*) INTO v_reminders FROM reminders
      WHERE user_id = p_user_id
        AND COALESCE(is_completed, false) = false
        AND entity_type IN ('lead','customer','quotation','task');
  END IF;
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  IF NOT (public.is_manager_or_above(auth.uid())
          AND public.is_same_tenant(p_from_user_id)
          AND public.is_same_tenant(p_to_user_id)) THEN
    RAISE EXCEPTION 'Permission denied';
  END IF;
